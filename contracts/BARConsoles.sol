...
contract BARConsoles is ERC721Enumerable, Ownable { // %%%%%%
    ...
    uint256 public MAX_SUPPLY = 5; //  2000 %%%%%%%%%%%%%%%%%%%%%%%%%
    uint256 public cost = 40000000000000000;  // cost of minting 
    ...
    // %%%%%%%%%%%%%%%%%%%
    string _name = 'BARConsolesTest';
    string _symbol = 'BARCT';
    string _uri = 'test';
    // %%%%%%%%%%%%%%%%%%%
    ...
    function mint(uint256 _num) public payable { // %%%%%%%%%%%%%
        ...
        if (preSaleStatus == true) {
            ...
            require(whitelist[msg.sender] >= _num, "Not on whitelist or maximum of 2 mints per address allowed"); // checks if white listed  & mint limit per address
            require(preSaleMintsTotal + _num <= 3, "Minting that many would exceed pre sale minting allocation"); // 250 in actual %%%%%%%%%%%%%%%%%%%
            whitelist[msg.sender] -= _num;
    ...
    function claim() public { // %%%%%%%%%%%%%%%
        ...
        require(claimStatus, "It's not time yet"); 
        
        uint tokenId = currentTokenId;
        require(tokenId <= MAX_SUPPLY, "All tokens have been minted");
        currentTokenId++;

        claimList[msg.sender] = false; // ensures they cannot claim a second time
        _mint(msg.sender, tokenId);
        emit TokenBought(tokenId);
    }
